...
**Status: real implementation (partial), milestone 2, extended milestone 4.** `cargo test -p aion-crypto` → 10/10 passing:

- `identity.rs` — Ed25519 P2P identity keypair (`docs/ARCHITECTURE.md`'s Identity Separation section): generate, sign, standalone `verify()` for a received signature, and `to_bytes()` for deliberate cross-crate identity bridging (used by `crates/aion-p2p`'s `keypair_from_identity` so a node's P2P PeerID is genuinely the same Ed25519 key as its `aion_crypto::Identity`, not two unrelated keys). Tests cover the roundtrip, tampered-payload rejection, cross-identity rejection, deterministic reconstruction from seed bytes, and the `to_bytes`/`from_bytes` export-reimport roundtrip.
