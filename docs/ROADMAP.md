...
| 5 | Verification | **Started** — `crates/aion-verifier`: L1 (commitment, wrapping `aion-crypto`) and L2 (replicated-execution numeric-tolerance clustering, distinguishing nondeterminism from divergence) implemented and tested in real Rust, ported from and pinned against the validated `simulations/poig_sim` Python reference. NOT done: L3 (deterministic-execution harness), L4 (optimistic dispute/bisection), L5 (ZKML), L6 (multi-validator quorum — exists in Python only, not yet ported) |
