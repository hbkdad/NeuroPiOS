...
L3 (deterministic-execution harness) in `crates/aion-verifier`, or the actual L4 bisection arbitration protocol behind `dispute.rs` — both are blocked on having a real job-execution pipeline to operate over (Phase 4), which doesn't exist in this dev container (no GPU/inference runtime). AutoNAT/circuit relay (NAT traversal) in `crates/aion-p2p` is the next concrete P2P gap now that 3-node discovery is resolved.
