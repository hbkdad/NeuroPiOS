...
| 5 | Verification | **Started** — `crates/aion-verifier`: L1 (commitment, wrapping `aion-crypto`), L2 (replicated-execution numeric-tolerance clustering, distinguishing nondeterminism from divergence), and L6 (commit/reveal weighted quorum, upgraded to real SHA-256 commit/reveal via `aion-crypto` vs. the Python reference's simplified hash) implemented and tested in real Rust, ported from and pinned against the validated `simulations/poig_sim` Python reference — including the honestly-proven majority-collusion residual risk, not just the flattering minority-blocked case. NOT done: L3 (deterministic-execution harness), L4 (optimistic dispute/bisection), L5 (ZKML) |
