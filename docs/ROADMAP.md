...
| 3 | Real P2P node | **MVP criterion met** — `crates/aion-protocol` (job object, nonce replay protection, no-double-settle state machine), `crates/aion-crypto` (Ed25519 identity, commit/reveal, cross-crate identity export), `crates/aion-node` (real hardware detection via `sysinfo`, Node Safety resource caps defaulting to locked-down, composable roles, owns and drives a real `aion-p2p` swarm built from its own identity), and `crates/aion-p2p` (real GossipSub+Identify+Kademlia+ConnectionLimits swarm over TCP+Noise+Yamux, peer scoring and connection-limit enforcement both active and tested) — all real Rust, clippy-clean, fmt-clean. Milestone 12 resolved the previously-open 3-node relayed Kademlia discovery bug (root cause: `libp2p_swarm::Config`'s default `idle_connection_timeout` of `Duration::ZERO` closed connections before in-flight FIND_NODE responses were flushed; fixed with an explicit nonzero timeout) — "three machines discover each other" (the Phase 3 MVP exit criterion) is now real and tested. NOT done: AutoNAT/relay (NAT traversal) |
...
The actual L4 bisection arbitration protocol behind `dispute.rs`, or AutoNAT/circuit relay (NAT traversal) in `crates/aion-p2p` — the bisection protocol is blocked on having a real job-execution pipeline to bisect over (Phase 4, no GPU/inference runtime in this dev container), while AutoNAT/relay is not blocked and is the next concrete, buildable P2P gap now that 3-node discovery is resolved.

Second-priority: `docs/security/ECONOMIC-ATTACKS.md`'s remaining unsimulated rows — treasury drain (governance/admin-key compromise) and front-running are both harder to model meaningfully without the contracts/settlement layer that doesn't exist yet (Phase 10); worth revisiting once that phase starts rather than forcing a premature abstraction now.
