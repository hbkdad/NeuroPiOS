...
| Off-chain PoIG number trusted blindly on-chain | **Open risk, unresolved in this milestone.** Needs either (a) a verifiable off-chain-compute attestation path, or (b) moving the core scoring arithmetic on-chain/into a rollup where it inherits chain security. Flagged explicitly rather than hand-waved — this is the single largest unresolved architectural question carried out of this milestone. | High until resolved; must be addressed before any real-value settlement (Stage C+) |
...
The single highest-priority unresolved item is **off-chain PoIG trust in on-chain settlement** (table above) — every other mitigation in this document assumes that gap is closed before real value moves. This is the top item for the next research/specification milestone, not something to defer indefinitely.
